...
export default {
	...
	parseData: function (res) {									//数据分析
		return {
			data: res.result.data,				//分析无分页的数据字段结构
			rows: res.result.data,		//分析行数据字段结构
			total: res.result.total,		//分析总数字段结构
			summary: res.result.summary,	//分析合计行字段结构
			msg: res.error_message,			//分析描述字段结构
			code: res.error_code				//分析状态字段结构
		}
	},
	request: {							//请求规定字段
		page: 'page',					//规定当前分页字段
		pageSize: 'page_count',			//规定一页条数字段
		prop: 'prop',					//规定排序字段名字段
